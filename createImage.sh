...
apt-get install -y isolinux syslinux-common syslinux-efi xorriso
...
#dd a blank image #a dvd is 4.7 gig
dd if=/dev/zero of="${myBuildHome}/image.img" bs=1M count=4699
...
printf "+256M\n"  # Last sector 256 megabytes for EFI partition
printf "t\n" #Partition type
printf "0b\n" #W95 FAT32
printf "n\n" # Add a new partition
printf "p\n" # Primary partition
...
#dd uboot spl
...
elif [ "$theArchMkImage" = "x86" ]; then
	mkfs.vfat -F 32 -s 1 /dev/loop321p1
	mkfs.ext2 -L "${DISKLABEL}" -F /dev/loop321p2
	...
	mkdir "${myBuildHome}/mount/extlinux"
	echo "MENU TITLE Boot Menu" > mount/extlinux/extlinux.cfg
	echo "PROMPT 1" >> mount/extlinux/extlinux.cfg
	echo "DEFAULT 1" >> mount/extlinux/extlinux.cfg
	echo "" >> mount/extlinux/extlinux.cfg
	echo "label 1" >> mount/extlinux/extlinux.cfg
	echo "    MENU LABEL Devuan testing" >> mount/extlinux/extlinux.cfg
	#setting the path to the symlink /vmlinux doesn't work
	echo "    KERNEL /boot/$(basename "$(find rootfs/boot/vmlinuz-*)")" >> mount/extlinux/extlinux.cfg
	echo "    APPEND initrd=/boot/$(basename "$(find rootfs/boot/initrd.img*)") root=LABEL=${DISKLABEL} init=/sbin/init-overlay.sh" >> mount/extlinux/extlinux.cfg
	echo "    TIMEOUT 1" >> mount/extlinux/extlinux.cfg

	#create script.bin
	#"${myBuildExtractDest}/u-boot-"*/tools/mkimage -A "$theArchMkImage" -O linux -T script -C none -n "U-Boot boot script" -d "${myBuildsDir}"/uboot/boot.cmd.x86 "${myBuildHome}/mount/boot.scr"
	#copy boot.cmd 
	#cp -a "${myBuildsDir}"/uboot/boot.cmd.x86 "${myBuildHome}/mount/boot.cmd"
	umount "${myBuildHome}/mount"

	#mount ext2 partition
	mount -t ext2 /dev/loop321p2 "${myBuildHome}/mount"
else
	mkfs.ext2 -L "${DISKLABEL}" -F /dev/loop321p1
	mkdir "${myBuildHome}/mount"
...
if [ "$theArchMkImage" != "x86" ]; then
	...
	#create script.bin
	"${myBuildExtractDest}/u-boot-"*/tools/mkimage -A "$theArchMkImage" -O linux -T script -C none -n "U-Boot boot script" -d "${myBuildsDir}"/uboot/boot.cmd.arm "${myBuildHome}/mount/boot/boot.scr"

	#copy boot.cmd.arm 
	cp -a "${myBuildsDir}"/uboot/boot.cmd.arm "${myBuildHome}/mount/boot/boot.cmd"
